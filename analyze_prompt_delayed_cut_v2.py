...
if event_type == "atmoNC":
    output_path_del = (output_path + "delayed_cut_{0:.0f}nsto{1:.0f}ms_mult{2:d}_{3:.0f}PEto{4:.0f}PE_dist{5:.0f}mm_"
...
""" delayed energy """
# number of events, where nPE_smeared pass delayed energy cut (and time cut and multiplicity cut):
number_delayed_energy_pass_reco = 0
# number of events, where nPE_smeared is rejected by delayed energy cut:
number_delayed_energy_rejected_reco = 0
# number of events, where nPE before recon. pass delayed energy cut:
number_delayed_energy_pass_MC = 0
# number of events, where nPE before recon. is rejected by delayed energy cut:
...
# array, where all nPE_smeared values are stored:
array_PE_smeared_delayed_energy = []
...
for filenumber in range(start_number, stop_number+1):
    ...
    for event in range(number_events):
        ...
        if not flag_read_hittime_from_file:
            ...
            npe_per_hittime_save = [min_time_total, max_time_total, binwidth]
        ...
        """ do delayed energy cut on real data (reconstructed) and on ideal (MC truth) data: """
        for cut_type in range(2):
            ...
            if cut_type == 0:
                ...
                sigma_energy_resolution_pe = NC_background_functions.energy_resolution_pe(nPE_del_MC)
                # smear nPE_del_MC with sigma_energy_resolution_pe to get reconstructed energy:
                nPE_del_smeared = np.random.normal(nPE_del_MC, sigma_energy_resolution_pe)

                """ store nPE_del_smeared in array_PE_smeared_delayed_energy: """
                array_PE_smeared_delayed_energy.append(nPE_del_smeared)

                # check efficiency of delayed energy cut:
                if (min_PE_delayed <= nPE_del_smeared <= max_PE_delayed and
                        (nPE_del_MC < min_PE_delayed or nPE_del_MC > max_PE_delayed)):
                    # nPE_del_smeared pass cut, but nPE_del_MC is rejected:
                    number_delayed_energy_toomuch += 1
                elif ((nPE_del_smeared < min_PE_delayed or nPE_del_smeared > max_PE_delayed) and
                      min_PE_delayed <= nPE_del_MC <= max_PE_delayed):
                    # nPE_del_smeared is rejected, but nPE_del_MC pass cut:
                    number_delayed_energy_tooless += 1

                if min_PE_delayed <= nPE_del_MC <= max_PE_delayed:
                    # nPE_del_MC pass cut:
                    number_delayed_energy_pass_MC += 1
                else:
                    # nPE_del_MC is rejected:
                    number_delayed_energy_rejected_MC += 1

                if min_PE_delayed <= nPE_del_smeared <= max_PE_delayed:
                    # nPE_del_smeared pass cut:
                    number_delayed_energy_pass_reco += 1
                else:
                    # nPE_del_smeared is rejected:
                ...
                # loop over n to get kinetic energy (kine) of the gamma (pdgig==22) that corresponds to the captured
...
""" build histogram from array_PE_smeared_delayed_energy: """
h4 = plt.figure(4, figsize=(15, 8))
bin_width = 100
bins_PE_del = np.arange(0, 17000+bin_width, bin_width)
values4, bins4, patches4 = plt.hist(array_PE_smeared_delayed_energy, bins_PE_del, align='mid', histtype='step',
                                    linewidth='1.5',
                                    label='entries = {0:d},\n'
                                          'events passing cut = {1:d},\n'
                                          'events rejected by cut = {2:d}'
...
plt.vlines(min_PE_delayed, ymin=0.0, ymax=(max(values4)+max(values4)*0.1), colors='k', linestyles='solid',
           label='lower edge of delayed energy window = {0:.1f} PE'.format(min_PE_delayed))
plt.vlines(max_PE_delayed, ymin=0.0, ymax=(max(values4)+max(values4)*0.1), colors='k', linestyles='dashed',
           label='upper edge of delayed energy window = {0:.1f} PE'.format(max_PE_delayed))
plt.xlim(xmin=0.0, xmax=17000)
plt.xlabel("number of PE in delayed signal")
plt.ylabel("number of events per bin (bin-width = {0:d} PE)".format(bin_width))
plt.title("Reconstructed energy of delayed signals of {0} events".format(event_type))
plt.legend()
plt.grid()
plt.savefig(output_path_del + "histo_delayed_energy_{0}_{1:.0f}PE_to_{2:.0f}PE_{3}.png"
            .format(event_type, min_PE_delayed, max_PE_delayed, analysis_part))
plt.close()

""" save different number of events of the delayed energy cut to txt file: """
np.savetxt(output_path_del + "numbers_delayed_energy_cut_{0}_{1:.0f}PE_to_{2:.0f}PE_{3:d}.txt"
           .format(event_type, min_PE_delayed, max_PE_delayed, analysis_part),
           ...
           header='number of events from analyze_prompt_delayed_cut_v2.py ({0}):'
                  ...
                  '\nanalyzed files: user_{1}_{4:d}.root to user_{1}_{5:d}.root'
                  '\n{1} events are analyzed for nPE_min = {2:.1f} PE to nPE_max = {3:.1f} PE.'
                  '\nValues below:'
                  '\nnumber of total events,'
                  '\nnumber of events without initial particles,'
                  '\nnumber of events that pass multiplicity cut (these events were analyzed),'
                  '\nnumber of events that pass delayed energy cut on PE_smeared,'
                  '\nnumber of events that are rejected by delayed energy cut on PE_smeared,'
                  '\nnumber of events that pass delayed energy cut on PE_MCtruth,'
                  '\nnumber of events that are rejected by delayed energy cut on PE_MCtruth,'
                  '\nnumber of events counted too much (PE_smeared pass, but PE_MCtruth is rejected),'
                  '\nnumber of events counted too less (PE_smeared is rejected, but PE_MCtruth pass):'
                  .format(now, event_type, min_PE_delayed, max_PE_delayed, start_number, stop_number))
...
np.savetxt(output_path + 'filenumber_evtID_delayed_cut_{0}_{1:.0f}ns_to_{2:.0f}ms_mult{3:d}_{4:.0f}PE_{5:.0f}PE_'
                         'dist{6:.0f}mm_R{7:.0f}mm_{8:d}.txt'
           .format(event_type, time_cut_min, time_cut_max/1000000.0, multiplicity, min_PE_delayed, max_PE_delayed,
                   distance_cut, R_cut_delayed_mm, analysis_part),
...
np.savetxt(output_path + 'filenumber_evtID_delayed_cut_MCtruth_{0}_{1:.0f}ns_to_{2:.0f}ms_mult{3:d}_{4:.0f}PE_'
                         '{5:.0f}PE_dist{6:.0f}mm_R{7:.0f}mm_{8:d}.txt'
           .format(event_type, time_cut_min, time_cut_max/1000000.0, multiplicity, min_PE_delayed, max_PE_delayed,
                   distance_cut, R_cut_delayed_mm, analysis_part),
