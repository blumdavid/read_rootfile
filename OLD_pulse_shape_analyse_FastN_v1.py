""" script to do pulse shape analysis of the fast neutron background:
    ...
    Neutron hittimes from script OLD_hittime_distribution_fastneutron.py, that are saved in folder
    ...
    of positron hittimes) the results from script OLD_pulse_shape_analysis_v1.py (summarized in file PSD_results.ods) are
    taken.
    ...
    1.  calculate the tail-to-total values for each neutron hittime distribution with function pulse_shape() also used
        in script OLD_pulse_shape_analysis_v1.py for start and stop value of tail given by PSD_results.ods.
    ...
    OLD_pulse_shape_analysis_v1.py
...
tot_value_positron = np.array([0.00699])

# best positron (IBD) efficiencies in %:
best_positron_efficiencies = np.array([6.82])

# corresponding NC (IBD-like) efficiencies in %:
NC_efficiencies = np.array([97.14])
...
for index in range(len(start_tail)):
    ...
    print("number of analyzed FN events = {0:.0f}".format(len(array_tot_ratio_neutron)))
